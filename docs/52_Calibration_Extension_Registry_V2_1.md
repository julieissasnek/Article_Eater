...
### VF-Series — CALIBRATED by Doc 64 (VF-II)

| Template | Name | Maturity | Calibration | Status | Source |
|----------|------|----------|-------------|--------|--------|
| **VF1** | Contour PE & Curvature-Angularity | Supported | ~ Partial | CCI metric (SD of curvature distribution). Goldilocks: smooth 0.15–0.40 (approach), moderate 0.40–0.80 (optimal), angular 0.80–1.20 (context-dependent). Effect d ≈ 0.45 acute, ~0.22 at 4 weeks (habituation τ ≈ 12–18 days, floor 40%). Knowledge-meaning modifier: k_expertise 0.0–0.6. Context-specific CCI ranges (healthcare 0.20–0.50; cultural 0.40–1.50). | Doc 64 §Cal-1 |
| **VF2** | Visual Rhythm & Scaling Hierarchy | Preliminary | ~ Partial | SRV (CV of element spacing): peak engagement 0.12–0.25. SCI (1 − CV of edge density across 5 frequency bands): rich >0.80; good 0.60–0.80; sparse <0.40. Multi-scale nourishment = T1 × VF2 × SCI. Auditory analogy grounded but not directly validated architecturally. | Doc 64 §Cal-2 |
| **VF3** | Spatial Proportions & Processing Mode | Supported | ~ Partial (deepened) | R_h Goldilocks (Doc 54) + transition enhancement: PE_transition = 0.55 × ln(R_h_new/R_h_old). Contour × proportion interaction: additive (Vartanian VR). CREA2B linkage: single causal chain, not parallel. Corridor→atrium d ≈ 0.50 creative broadening. | Doc 54, 64 §Cal-3 |
...
| **VF1–VF3** | **~ Partial** | **Doc 64** |
| COL1–COL2 | ~ Partial / ✓ Good | Doc 63 |
| OLF1 | ✗ Uncalibrated | — |

**ALL NINE template series now calibrated. Only OLF1 remains uncalibrated (single template, low priority).**
...
### ADDENDUM (V2.1): VF-II Visual Form Calibration (from Doc 64)

| Template | Calibration | Key Parameters | Source |
|----------|-------------|----------------|--------|
| **VF1** | ~ Partial | CCI metric (SD curvature distribution). Goldilocks: smooth 0.15–0.40, moderate 0.40–0.80, angular-mixed 0.80–1.20, angular 1.20–1.80+. Effect d ≈ 0.45 (acute), floor d ≈ 0.18 (habituated). Habituation: d(t) = d_acute × [0.40 + 0.60 × e^(−t/τ)], τ ≈ 12–18 days. Knowledge-meaning modifier: d × (1 − 0.6 × expertise × context). | Doc 64 §Cal-1 |
| **VF2** | ~ Partial | SRV (CV element spacing) Goldilocks: 0.12–0.25 peak (from auditory analogy). SCI = 1 − CV(edge density across 5 bands): rich >0.80; good 0.60–0.80; sparse 0.20–0.40. Multi-scale nourishment = T1 × VF2 × SCI. | Doc 64 §Cal-2 |
| **VF3** | ~ Partial (deepened) | Transition enhancement: PE_transition = 0.55 × ln(R_h_new/R_h_old). Corridor→atrium (2.7×) → d ≈ 0.50 creative broadening. VF1×VF3 additive. CREA2B = single chain through VF3, not parallel computation. | Doc 64 §Cal-3 |

**CALIBRATION SWEEP COMPLETE: All nine template series (L, MAT, TP, SOC, CREA, VIEW, SC, COL, VF) now calibrated.**

### Updated Calibration Status Summary (V2.1)
...
### Updated Priority Queue (V2.1)

| Priority | Panel | What It Delivers | Impact | Prerequisite Docs |
|----------|-------|-----------------|--------|-------------------|
| 1 | **SOC-III or CREA-III** | See diminishing returns analysis | A8 or A9 deepening | Docs 57, 58 |
| 2 | (the other of SOC-III/CREA-III) | | | |
| 3 | **VF-III** | SRV psychophysical validation, SCI empirical test, transition experiment | A6 deepening | Doc 64 |
| 4 | **AGE-II** | Dementia-specific, cascade validation | Cross-cutting | Doc 60 |
| 5 | **DEV-II** | ADHD parameters, U-curve validation | Cross-cutting | Doc 61 |
| 6 | **SC-III** | Urban-scale, compositional shape validation | A3 deepening | Doc 62 |

### VF-II Cross-Template Encoding Patterns

**CCI Context Lookup**: Templates involving visual form assessment should include `contour_curvature_index: number` and `building_context: healthcare | school | office | cultural | memorial`. CCI recommendations vary by context per the Cal-1 context table.

**Exposure Habituation for Visual Form**: VF1 habituation follows the same exponential decay pattern as COL2 (Doc 63). Templates should include `exposure_duration_weeks: number` and compute `d_effective = d_acute × [0.40 + 0.60 × e^(−t/τ)]` where τ ≈ 12–18 days. The floor factor 0.40 represents the persistent perceptual component.

**Multi-Scale Visual Assessment**: VF2 establishes that visual form quality requires assessment at MULTIPLE spatial scales. Templates involving visual quality should compute SCI from edge density at 5 frequency bands. Flag when SCI < 0.40 (visual starvation risk).

**VF3-CREA2B Single Chain**: When computing creativity impact of spatial proportions, the system should trace VF3 → affect → CREA2B, not compute both independently. The CREA3 motor pathway remains independent and additive.

---

*Calibration & Extension Registry — Document 52 V2.1*
*February 17, 2026*
*Update: VF-II calibration (Doc 64) + CREA-III deepening (Doc 65) — ALL NINE TEMPLATE SERIES CALIBRATED*
*Template count: 73 (72 + CREA4)*
*Next update trigger: Post-audit revision*

### ADDENDUM (V2.1b): CREA-III Creative Deepening (from Doc 65)

**New Template:**

| Template | Name | Maturity | Calibration | Key Parameters | Source |
|----------|------|----------|-------------|----------------|--------|
| **CREA4** | Collaborative Creativity Architecture | Supported-preliminary | ~ Partial | Alternation cycle: 8–15 min individual → 5–10 min group → 3–5 min incubation. Optimal group 3–6 (Dunbar sympathy). Transition time <30s. Display density ≥1.5 m²/person writable. Psychological safety: multiplicative moderator (d_arch = d_max × safety_baseline). Production blocking reduction ~40–60% with spatial alternation support. | Doc 65 §CREA4 |

**Updated CREA2 interaction matrix:**

| Pathway Combo | Sub-additivity | Convergent Penalty | Maturity |
|---------------|---------------|-------------------|----------|
| A alone (noise) | 1.00 | 1.00 | supported |
| B alone (ceiling) | 1.00 | 1.00 | supported |
| C alone (dim) | 1.00 | 1.00 | supported |
| A+B (noise + ceiling) | 0.84 | 1.15 | expert est. |
| A+C (noise + dim) | 0.76 | 1.60 | supported |
| B+C (ceiling + dim) | 0.80 | 1.10 | expert est. |
| A+B+C (all three) | 0.70 | 1.80 | expert est. |

**Design rule**: Generative zone → A+B (best ROI). Switching zone → B or C alone. Evaluative zone → none.

**Updated phase-environment mapping:** See Doc 65 §Cal-5 for specific environmental parameters per creative phase.

### Updated Priority Queue (V2.1b)

| Priority | Panel | What It Delivers | Impact | Prerequisite Docs |
|----------|-------|-----------------|--------|-------------------|
| **1** | **DOC 66: COMPREHENSIVE PROJECT AUDIT** | Ruthless assessment of entire system: template coherence, coverage honesty, PE framework integrity, cross-panel consistency, actionable capability | **Critical inflection point** | All docs |
| 2 | **SOC-III** | Multicultural design, healthcare, digital-physical privacy | A8 deepening | Doc 57 |
| 3 | **VF-III** | SRV psychophysical validation, SCI empirical test | A6 deepening | Doc 64 |
| 4 | **AGE-II** | Dementia-specific, cascade validation | Cross-cutting | Doc 60 |
| 5 | **DEV-II** | ADHD parameters, U-curve validation | Cross-cutting | Doc 61 |
| 6 | **SC-III** | Urban-scale, compositional shape validation | A3 deepening | Doc 62 |

### CREA-III Cross-Template Encoding Patterns

**Interaction Matrix Lookup**: Replace CREA2's single sub-additivity coefficient with the 7-cell lookup table. When multiple CREA2 pathways are active, look up the specific combination's sub-additivity and convergent penalty multiplier.

**Workspace Mode Tag**: Templates involving creative environments should include `workspace_mode: generative_only | divergent_convergent_switching | balanced` and apply the corresponding pathway recommendation from the design rule.

**CREA4 Alternation Cycle**: When `context: "collaborative_creative"`, activate the alternation cycle parameters. Require spatial configuration with individual stations (<15m from commons), collaborative commons (sized for 4–6), and transition space. Flag when transition time > 30s.

**Psychological Safety Multiplier**: All creativity-related architectural effects should be scaled by `organizational_safety_baseline: 0.0–1.0`. When safety_baseline < 0.3, flag that architectural creativity features will have minimal effect.
