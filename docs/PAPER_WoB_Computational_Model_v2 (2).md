...
A natural objection arises here, and it must be answered directly: *why not simply put the full mechanism chain into the Bayesian network?* If Daylight → ipRGC → Raphe Nuclei → 5-HT Synthesis → Positive Mood is a causal chain, why not represent it as a chain of BN nodes rather than compressing it to a single edge? A causal connection is a causal connection, one might think — so why can the web represent this chain but the BN cannot?

The answer has four parts, each independently sufficient.

**First, the intermediate nodes are not observable.** A Bayesian network node requires a probability distribution over states — P(Raphe Nuclei = active | ipRGC = stimulated) — and that distribution must be estimable from data. For the endpoints (daylight level, mood rating), we can measure these in building occupants. For the intermediate neural steps, we generally cannot. No one is performing jugular vein cannulation on office workers to measure 5-HIAA turnover while varying their daylight exposure. The mechanism chain's intermediate steps exist as *theoretical claims about unobserved processes*, not as *random variables with measurable probability distributions*. The web can represent theoretical claims with Toulmin provenance and graded credences. The BN requires data. This asymmetry is fundamental: the web traffics in epistemic states (what the community believes about mechanisms); the BN traffics in aleatory states (what the population frequencies actually are). The intermediate mechanism steps belong to the epistemic domain — they are things we believe happen, with varying degrees of confidence and different kinds of evidence — not to the aleatory domain.

**Second, the intermediate nodes lack the conditional independence structure that makes BN inference tractable.** Pearl's do-calculus and the efficient inference algorithms for BNs (variable elimination, belief propagation) depend critically on the conditional independence assumptions encoded in the DAG: a node is conditionally independent of its non-descendants given its parents. For the observable endpoints (daylight, mood), these independence assumptions can be tested and defended. For the intermediate neural mechanisms, they cannot — because we do not know what else influences raphe nuclei activity in a building occupant (stress, caffeine intake, time of day, medications, genetic variation in tryptophan hydroxylase). Adding unobserved nodes to a BN without knowing their full parent sets violates the conditional independence assumptions that make the BN's computations valid. The BN would produce numbers, but the numbers would be wrong — the confounding structure at the intermediate nodes is unknown and uncontrolled.

**Third, the edges in the mechanism chain carry different epistemological characters that the BN cannot represent.** In the web, the edge from Neuromodulation to T2_DAYLIGHT_5HT carries a MECHANISM bridge warrant (α = 0.75, ceiling = 0.75) — meaning the pathway has been traced anatomically. The edge from T2_DAYLIGHT_5HT to mood carries an EMPIRICAL_COVARIANCE bridge warrant (α = 0.70, ceiling = 0.70) — meaning we have replicated association data but the in-building causal link is unconfirmed. In a BN, both edges would carry the same kind of information: conditional probability tables. The distinction between "anatomically traced pathway" and "replicated correlation" — which determines what research would improve the estimate and how much confidence is warranted — is invisible. Putting the chain in the BN would preserve the chain's topology but erase the epistemological structure that makes the chain scientifically interpretable.

**Fourth, and most fundamentally, the web's mechanism chain and the BN's causal edge answer different questions.** The mechanism chain answers: *through what process does daylight affect mood, and how confident are we in each step?* The BN edge answers: *if an architect sets daylight to a high level in this building, what is the probability that occupant mood will be positive, controlling for confounders?* These are different questions. The first is about the structure of a scientific explanation. The second is about the expected outcome of an intervention. The do-calculus that makes the BN's interventional answer rigorous operates over *observable, manipulable variables* — precisely the endpoints. It cannot operate over theoretical intermediate nodes whose manipulation is either impossible (you cannot intervene on raphe nuclei activity by adjusting a building) or meaningless (the architect controls daylight, not 5-HT synthesis). The BN's power lies in computing interventional and counterfactual consequences for variables that someone can actually observe and act upon. The mechanism chain explains *why* those consequences obtain; the BN computes *what* they are.
...
The BN has **no concept of explanation**. It can compute P(Mood = positive | do(Daylight = high)) = 0.70, but it cannot explain *why* daylight improves mood. The mechanism (daylight → retinal stimulation → raphe nuclei → tryptophan hydroxylase → serotonin synthesis → positive processing bias) is not represented in the BN — and, as argued in §3.7, *cannot* be represented there, because the intermediate nodes are unobservable in practice, lack the conditional independence structure BN inference requires, carry epistemological distinctions invisible to the BN's representational format, and describe processes that no architect can manipulate. The BN has a single edge "Daylight → Mood" with a CPT. The *why* lives in the web.
...
A reader may wonder: why not simply put the mechanism chain *into* the Bayesian network, preserving its internal structure? The answer (developed fully in §3.7) is that the intermediate nodes — raphe nuclei, 5-HT synthesis, circadian entrainment — are not observable in building occupants, lack the conditional independence structure that BN inference algorithms require, carry epistemological distinctions (MECHANISM vs. EMPIRICAL_COVARIANCE warrants) that the BN format cannot encode, and describe processes that no architect can manipulate. The BN's computational power — do-calculus, counterfactual reasoning — operates over variables that can be observed (mood ratings) and intervened upon (daylight levels). The mechanism chain explains *why* daylight affects mood; the BN computes *what happens if* an architect changes daylight. These are different questions requiring different representational formats, and the projection function is the formal bridge between them.
...
**Figure 2.** Projection Loss: From Mechanism Chain (Web) to Single Edge (BN). A four-step mechanism chain — daylight → retinal ipRGC → raphe nuclei → 5-HT synthesis → positive mood — is compressed into a single BN edge (Daylight → Mood) with a CPT. All intermediate mechanism steps, warrant types, Toulmin provenance, and competing accounts are lost in the projection. The compression is not a convenience but a necessity: the intermediate nodes are unobservable in practice, lack the conditional independence structure BN inference requires, carry epistemological distinctions the BN cannot represent, and answer a different question (mechanism explanation vs. interventional prediction). See §3.7 for the full argument. *(See companion file: Figure2_Projection_Loss.svg)*
