...
# ============================================================================
# FASTAPI DEPENDENCIES (JWT + ROLE CHECKS)
# ============================================================================

# Dedicated HTTP bearer security for auth dependencies (separate from app.main).
security = HTTPBearer(auto_error=False)

def _get_db_path() -> str:
    """Resolve the auth database path, preferring AE_DB env var."""
    return os.environ.get("AE_DB", "./ae.db")
...
async def get_current_user_dep(
    ...
    Returns None if no credentials are provided (anonymous access),
    but raises 401/403 for invalid or inactive users.
    """
    if not credentials:
        return None
    ...
    user = get_user_by_id(token_data.user_id, db_path=_get_db_path())
    if not user:
...
async def require_admin(user: User = Depends(get_current_user_dep)) -> User:
    """Dependency that enforces an 'admin' role for the current user."""
    if not user or user.role.lower() != "admin":
        raise HTTPException(status_code=403, detail="Admin privileges required")
    return user


async def require_researcher_or_admin(user: User = Depends(get_current_user_dep)) -> User:
    """Dependency that allows 'researcher' or 'admin' roles."""
    if not user or user.role.lower() not in ("admin", "researcher"):
