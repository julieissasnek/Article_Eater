...
def run_from_contract_bundle(
    *, in_dir: Path, out_dir: Path, profile: str, hitl: str
    ...
    llm_blocked = False
    if text:
        has_llm_key = bool(os.environ.get("GOOGLE_API_KEY") or os.environ.get("OPENAI_API_KEY"))
        if not has_llm_key:
            llm_blocked = True
    ...
    for idx, finding in enumerate(findings, start=1):
        if isinstance(finding, dict):
            ...
            statement = finding.get("finding_text") or finding.get("statement") or "Unspecified finding"
    ...
    result = {
        ...
        "quality": {"confidence": max([c["ae_confidence"] for c in claims], default=0.0), "blocking_issues": blocking, "warnings": []},
        "errors": errors,
    }
