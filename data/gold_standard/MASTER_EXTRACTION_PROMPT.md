You are extracting a scientific paper for the Article Eater gold standard corpus.

## STEP 1: CLASSIFY THE PAPER

You are analyzing a scientific paper to classify its type.
...
## STEP 2: EXTRACT USING THE CORRECT TEMPLATE

Based on your classification, use the corresponding extraction template below.

### IF EMPIRICAL_RESEARCH:
You are extracting structured findings from an EMPIRICAL RESEARCH paper.
...
ADDITIONAL ARTICLE-LEVEL FIELDS (required for all types):

10. **RESEARCH_QUESTION**: Quote or paraphrase the paper's explicit research question(s).
11. **HYPOTHESES**: List each stated hypothesis with id, statement, supported (true/false/null), evidence_findings.
12. **STIMULUS_DESCRIPTION**: Required for every empirical finding. Must include:
    - stimulus_image_available (true/false): Does the paper include figures showing the stimulus?
    - stimulus_image_description (max 500 chars): If yes, describe what you see in the figure.
...

{
  ...
  "research_question": "What is the effect of ceiling height on creative ideation?",
  "hypotheses": [
    {
      "id": "H1",
      "statement": "Higher ceilings will increase creative ideation scores",
      "supported": true,
      "evidence_findings": ["F1"]
    }
  ],
  "n_findings": 0,
        ...
        "components": [
          {
            ...
            "category": "spatial",
            "essential": true,
            "details": "3m high condition vs 2.4m low condition"
          }
        ],
        "delivery_method": "in_situ",
        "duration_seconds": null,
        "stimulus_image_available": true,
        "stimulus_image_description": "Figure 1 shows the two experimental rooms: (a) high-ceiling room (3m) with standard furniture, fluorescent lighting, white walls; (b) low-ceiling room (2.4m) with identical furniture and lighting."
      },
...
### IF META_ANALYSIS:
You are extracting findings from a META-ANALYSIS paper.
...
### IF NARRATIVE_REVIEW:
You are extracting findings from a NARRATIVE REVIEW paper.
...
### IF THEORETICAL:
You are extracting propositions from a THEORETICAL paper.
...
### IF QUALITATIVE:
You are extracting themes from a QUALITATIVE RESEARCH paper.
...
You are extracting findings from a SYSTEMATIC REVIEW paper.
...
You are extracting validation data from an INSTRUMENT/METHODS paper.
