...
# UI configuration endpoint for frontend mode flags
@app.get("/ui/config", tags=["ui"])
async def ui_config():
    """Expose minimal UI configuration for frontend clients.

    Currently includes:
    - demo_mode: whether search UI is allowed to show mock/demo results.
    - engine_use_mocks: whether backend engine is configured to use mocks.
    - eager: current eager policy flags (for future UI toggles).
    """
    policy = get_policy()
    ui_cfg = policy.get("ui", {}) or {}
    engine_cfg = policy.get("engine", {}) or {}
    eager_cfg = policy.get("eager", {}) or {}
    return {
        "demo_mode": bool(ui_cfg.get("demo_mode", False)),
        "engine_use_mocks": bool(engine_cfg.get("use_mocks", False)),
        "eager": eager_cfg,
    }
