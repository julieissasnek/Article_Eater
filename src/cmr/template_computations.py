...
# =============================================================================
# REGISTRY OF ALL COMPUTE FUNCTIONS
# =============================================================================
...
TEMPLATE_COMPUTE_FUNCTIONS = {
    ...
    # Batch 3 (gap templates, Task 3.8)
...
def get_compute_function(display_id: str):
    """Get the compute function for a template by display_id."""
    return TEMPLATE_COMPUTE_FUNCTIONS.get(display_id)


def list_implemented_templates() -> List[str]:
    """List all implemented template display_ids."""
    return list(TEMPLATE_COMPUTE_FUNCTIONS.keys())

# =============================================================================
# GAP TEMPLATE COMPUTATIONS (T4, T6, T7, T10, T14, T15, T17, T18, T23, T28)
# =============================================================================

RISK_TO_WIS = {"low": 65.0, "moderate": 45.0, "high": 25.0}


def _gap_result(template: str, risk_level: str, description: str, inputs: dict) -> dict:
    return {
        "template": template,
        "risk_level": risk_level,
...
def compute_t4_attention_demand(task_intensity: float, noise_dba: float) -> dict:
    risk = "high" if task_intensity > 0.8 or noise_dba > 70 else "moderate" if task_intensity > 0.5 else "low"
    return _gap_result(
        "T4",
        risk,
        "Attention demand overload when intensity or ambient noise is high",
        {"task_intensity": task_intensity, "noise_dba": noise_dba},
    )


def compute_t6_cortisol_cascade(noise_dba: float, sleep_quality: float, control_perception: float) -> dict:
    if noise_dba > 75 or sleep_quality < 0.4:
        risk = "high"
    elif control_perception < 0.5:
        risk = "moderate"
    else:
        risk = "low"
    return _gap_result(
        "T6",
        risk,
        "Cortisol cascade triggered by noise, poor sleep, and low control",
        {
            "noise_dba": noise_dba,
            "sleep_quality": sleep_quality,
            "control_perception": control_perception,
...
def compute_t7_allostatic_load(stress_events: int, social_support: float, recovery_time: float) -> dict:
    risk = "low"
    if stress_events > 5 and social_support < 0.4:
        risk = "high"
    elif recovery_time < 8 and social_support < 0.7:
        risk = "moderate"
    return _gap_result(
        "T7",
        risk,
        "Allostatic load from frequent stress events with weak buffers",
        {
...
def compute_t10_sleep_consolidation(light_exposure: float, screen_time: float, bedtime_consistency: float) -> dict:
    risk = "high" if light_exposure > 650 or screen_time > 3 else "moderate" if bedtime_consistency < 0.5 else "low"
    return _gap_result(
        "T10",
        risk,
        "Sleep consolidation compromised by light/noise exposure or instability",
        {
...
def compute_t14_navigation_stress(navigation_complexity: float, signage_quality: float) -> dict:
    risk = "high" if navigation_complexity > 0.8 and signage_quality < 0.6 else "moderate" if navigation_complexity > 0.5 else "low"
    return _gap_result(
        "T14",
        risk,
        "Navigation stress arises when complexity is high and signage is poor",
        {"navigation_complexity": navigation_complexity, "signage_quality": signage_quality},
    )


def compute_t15_environmental_control(control_autonomy: float, mechanical_noise: float) -> dict:
    if control_autonomy < 0.3 or mechanical_noise > 55:
        risk = "high"
    elif control_autonomy < 0.6:
        risk = "moderate"
    else:
        risk = "low"
    return _gap_result(
        "T15",
        risk,
        "Environmental control lacks autonomy and mechanical noise is high",
        {"control_autonomy": control_autonomy, "mechanical_noise": mechanical_noise},
    )


def compute_t17_dopaminergic_novelty(novelty_density: float, prediction_error: float) -> dict:
    risk = "low"
    if novelty_density < 0.2 and prediction_error < 0.1:
        risk = "high"
    elif novelty_density < 0.4:
        risk = "moderate"
    return _gap_result(
        "T17",
        risk,
        "Dopaminergic novelty signal too low for intrinsic motivation",
        {"novelty_density": novelty_density, "prediction_error": prediction_error},
    )


def compute_t18_vestibular_spatial(acceleration_changes: float, walkway_smoothness: float) -> dict:
    if acceleration_changes > 1.0 and walkway_smoothness < 0.65:
        risk = "high"
    elif acceleration_changes > 0.5:
        risk = "moderate"
    else:
        risk = "low"
    return _gap_result(
        "T18",
        risk,
        "Vestibular spatial cohesion threatened by jerky acceleration or rough surfaces",
        {"acceleration_changes": acceleration_changes, "walkway_smoothness": walkway_smoothness},
    )


def compute_t23_context_memory(cue_density: float, support_structures: float) -> dict:
    risk = "high" if cue_density > 0.8 and support_structures < 0.4 else "moderate" if cue_density > 0.6 else "low"
    return _gap_result(
        "T23",
        risk,
        "Context memory overloaded when cues are dense and support structures weak",
        {"cue_density": cue_density, "support_structures": support_structures},
    )


def compute_t28_cognitive_offloading(assistive_tools: float, task_switching: float) -> dict:
    if task_switching > 0.6 and assistive_tools < 0.3:
        risk = "high"
    elif task_switching > 0.4:
        risk = "moderate"
    else:
        risk = "low"
    return _gap_result(
        "T28",
        risk,
        "Cognitive offloading suffers when switching is frequent without tools",
        {"assistive_tools": assistive_tools, "task_switching": task_switching},
    )
